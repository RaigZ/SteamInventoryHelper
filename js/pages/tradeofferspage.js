...
function getTradeCost(offerId) {
    $.ajax({
        ...
        data: {key: window._apikey, tradeofferid: offerId},
                    ...
                    success: function (response) {
                        if (response.success && response.lowest_price) {
                            ...
                            if (itemsToReceive.indexOf(response.hashName) !== -1) {
                                ...
                                $('.price-theirs-' + offerId).text(v_currencyformat(receivePrice * 100, 'USD'));
                            }
                            if (itemsToGive.indexOf(response.hashName) !== -1) {
                                givePrice += parseFloat(price);
                                $('.price-yours-' + offerId).text(v_currencyformat(givePrice * 100, 'USD'));
                            }
                        }
                    },
                    error: function () {}
...
function collectItems(elements, description) {
    ...
    elements.forEach(function (elem) {
        description.forEach(function (descItem) {
            if (descItem.classid == elem.classid && descItem.instanceid == elem.instanceid) {
                data.push(descItem.market_hash_name);
                return true;
            }
        });
    });
